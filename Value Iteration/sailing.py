...
class SailingGridworld(object):
    STAY, LEFT, DOWN, RIGHT, UP = 0, 1, 2, 3, 4
    NO_ACTIONS = 5
    ...
    def __init__(self, rock_penalty=-2, harbour_reward=10, wind_p=0.1):
        ...
        self.rewards = np.zeros((self.w, self.h, self.NO_ACTIONS))
        self.terminate = np.zeros((self.w, self.h, self.NO_ACTIONS))
        self.state = 0, 0
        ...
        # Die on rocks, no amtter the further action
        ...
        self.terminate[self.harbour_x, self.harbour_y, self.STAY] = 1
        self.rewards[self.harbour_x, self.harbour_y, self.STAY] = harbour_reward
    ...
    def _get_next_states_wind(self, state, action):
        ...
        transitions = [(desired_state, self.rewards[state[0], state[1], action],
                        self.terminate[state[0], state[1], action], 1-4*self.wind_p),
                       (desired_up, self.rewards[state[0], state[1], action],
                        self.terminate[state[0], state[1], action], self.wind_p),
                       (desired_down, self.rewards[state[0], state[1], action],
                        self.terminate[state[0], state[1], action], self.wind_p),
                       (desired_left, self.rewards[state[0], state[1], action],
                        self.terminate[state[0], state[1], action], self.wind_p),
                       (desired_right, self.rewards[state[0], state[1], action],
                        self.terminate[state[0], state[1], action], self.wind_p)]
    ...
    def render(self):
        ...
        if self.state is not None:
            ...
            boat = Polygon(xy=list(zip(boat_x, boat_y)), fill=True, facecolor="black")
    ...
    def _get_neighbouring_state(self, state, relative_pos):
        """Returns the next state to be reached when action is taken in state.
           Assumes everything ot be deterministic.
    ...
    def _get_possible_transitions_standard(self, state, action):
        ...
        if self.terminate[state[0], state[1], action]:
            return [(None, self.rewards[state[0], state[1], action], True, 1)]
        transitions = []
        action1 = (action-1) % self.NO_ACTIONS
        state1 = self._get_neighbouring_state(state, action1)
        reward1 = self.rewards[state[0], state[1], action]
        terminate1 = self.terminate[state[0], state[1], action]
        ...
        reward2 = self.rewards[state[0], state[1], action]
        terminate2 = self.terminate[state[0], state[1], action]
        transitions.append((state2, reward2, terminate2, self.wrong_action_prob))
        state3 = self._get_neighbouring_state(state, action)
        reward3 = self.rewards[state[0], state[1], action]
        terminate3 = self.terminate[state[0], state[1], action]
    ...
    def draw_values(self, values):
        ...
        for i, row in enumerate(values):
            ...
            for j, value in enumerate(row):
                ry = (j+0.8)/self.h
                self.ax.text(rx, ry, "{:.2f}".format(value), ha='center', va='center')
    ...
    def draw_actions(self, policy):
        ...
        pol_str[pol_str == str(self.STAY)] = "Stay"
        for i, row in enumerate(pol_str):
            rx = (i+0.5)/self.w
            for j, value in enumerate(row):
                ry = (j+0.2)/self.h
                self.ax.text(rx, ry, value, ha='center', va='center')
